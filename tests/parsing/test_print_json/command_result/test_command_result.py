...
PACKET_TYPE: Final = packets.PrintJSON
PRINT_JSON_TYPE: Final[enums.PrintJSONType] = PrintJSONType.COMMAND_RESULT

@pytest.mark.parametrize("data", TEST_DATA, ids=lambda p: p[0].name)
def test_model(data: tuple[Path, str]):
    packet = helper_test_parse_json(data[1], PACKET_TYPE)
